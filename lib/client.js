...
setTimeout(() => {
	...
	let authore = options.author || 'Prince Rudh'
	...
	return await Void.sendMessage(citel.chat,{sticker:{url:data}},{quoted:citel})
                ...
                events.plugins.map(async(command) => {
 ...
 Void.ev.on('group-participants.update', async(anu) => {
        ...
        Void.getName = (jid, withoutContact = false) => {
            ...
            /*
                Void.sendContact = async (jid, kon, quoted = '', opts = {}) => {
                    ...
                    for (let i of kon) {
                        list.push({
                            displayName:PrinceRudh,
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${global.ownerName}\nFN:${global.ownerName}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Rudhra userbot's owner.\nitem2.EMAIL;type=INTERNET:${global.email}\nitem2.X-ABLabel:Email\nitem3.URL:${global.github}\nitem3.X-ABLabel:GitHub\nitem4.ADR:;;${global.location};;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
                        })
                    }
        ...
        Void.ev.on('connection.update', async(update) => {
                ...
                if (connection === 'open') {
                    ...
                    for (let i of owner) {
                        Void.sendMessage(i + "@s.whatsapp.net", { text: `_Rudhra has been integrated._\n_Total Plugins : ${events.plugins.length}_\n_Mode: ${Config.WORKTYPE}_\n_Version:- 0.0.5_\n_Branch:- ${Config.BRANCH}_\n_Language: ${Config.LANG}_\n_Owner:- ${process.env.OWNER_NAME}_\n` })
                    }
                }
                if (connection === 'close') {
