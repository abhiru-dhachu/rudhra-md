...
const { prh1, tiny, fancytext, listall,cmd } = require('../lib/')
...
cmd({
        ...
        async(Void, citel, text) => {
            ...
            let username = await prh1.findOne({ id: citel.quoted.sender })
            var tname;
            ...
            return citel.reply(img,{packname:'Rudhra',author:'Quotely'},"sticker")
...
cmd({
        ...
        async(Void, citel, text) => {
            if (isNaN(text.split(" ")[0]) || !text) {
                ...
                listall("Rudhra Bot").forEach((txt, num) => {
                    text += `${(num += 1)} ${txt}\n`;
                });
