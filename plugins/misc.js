 const { tlang, getAdmin, prefix, Config, prh, fetchJson, runtime,cmd } = require('../lib')
...
async(Void, citel, text,{ isCreator }) => {
    if (!isCreator) return citel.reply(tlang().owner)
          let Group = await prh.findOne({ id: citel.chat })
            if (!Group) {
                await new prh({ id: citel.chat, welcome: text,events:'true' }).save()
                return citel.reply('Welcome added added for this group.')
            } else {
                await await prh.updateOne({ id: citel.chat }, { welcome:text ,events:'true'})
...
async(Void, citel, text,{ isCreator }) => {
    if (!isCreator) return citel.reply(tlang().owner)
          let Group = await prh.findOne({ id: citel.chat })
            if (!Group) {
                await new prh({ id: citel.chat, goodbye: text,events:'true' }).save()
                return citel.reply('Goodbye added for this group.');
            } else {
                await await prh.updateOne({ id: citel.chat }, { goodbye:text,events:'true' })
                 ...
                 default:
                     {
                         ...
                         let chatbott= await chatbot.findOne({ id: 'chatbot' })
                         await Void.sendButtonText(citel.chat, buttons, `Chatbot Status: ${chatbott.worktype} `, 'rudhra-md', citel);
                     }
             }
