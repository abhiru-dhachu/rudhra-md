 const { prh,prh1,cmd, getBuffer, tlang, prefix } = require('../lib')
 ...
 cmd({
     ...
     async(Void, citel, text,{ isCreator }) => {
        let zerogroup = (await prh.findOne({
            id: citel.chat,
        })) || (await new prh({
                id: citel.chat,
            })
	...
	const rudhra = "rudhra"
	const daily  = await eco.daily(citel.sender, rudhra, 2000); //give 500 for daily, can be changed
...
Version: 0.0.6`, citel);
 ...
 cmd({
	...
	if(!users) return citel.reply('Please give me user.')
        const balance  = await eco.balance(users, "rudhra")
        await eco.deduct(users, "rudhra", balance.wallet);
...
async(Void, citel, text,{ isCreator }) => {
    ...
    const balance  = await eco.balance(user, rudhra)
    switch (value) {
        case '1000':
        case '1':
        if (k > balance.wallet ) return citel.reply(`*_You need to pay 🪙100 to increase bank capacity ~ 1000 sp_*`);
          const deduct1 = await eco.deduct(user, rudhra, 100);
          const add1 = eco.giveCapacity(user, rudhra, 1000);
              let buttons = [{
                buttonId: `${prefix}wallet`,
                buttonText: {
                    displayText: "Wallet👛",
...
Version: 0.0.6`, citel);
        ...
        if (k < balance.wallet) return citel.reply(`*You need to pay 🪙1000 to increase bank capacity ~ 100000 sp*`);
          const deduct2 = await eco.deduct(user, rudhra, 1000);
          const add2 = eco.giveCapacity(user, rudhra, 100000);
              let buttonss = [{
                buttonId: `${prefix}wallet`,
                buttonText: {
                    displayText: "Wallet👛",
...
Version: 0.0.6`, citel);
        ...
        if (k < balance.wallet) return citel.reply(`You need to pay 🪙10000 to increase bank capacity ~ 1000 sp`);
           const deduct3 = await eco.deduct(user, rudhra, 10000);
           const add3 = eco.giveCapacity(user, rudhra, 10000000);
               let buttonst = [{
                buttonId: `${prefix}wallet`,
                buttonText: {
                    displayText: "Wallet👛",
 ...
 await citel.reply('*What are you trying to do📉*.')
    ...
    async(Void, citel, text,{ isCreator }) => {
      ...
      //  let users = citel.mentionedJid ? citel.mentionedJid[0] : citel.msg.contextInfo.participant || false;
        ...
        let d = parseInt(text)
        const deposit = await eco.deposit(citel.sender, "rudhra", d);
        const balance = await eco.balance(citel.sender, "rudhra")
...
Version: 0.0.6`, citel);
    ...
    async(Void, citel, text,{ isCreator }) => {
	let h = await eco.lb('rudhra',10);
	let str = `*Top ${h.length} users with more money in wallet.*\n`
	const { prh1 } = require('../lib');
	let arr = []
	 for(let i=0;i<h.length;i++){
            let username = await prh1.findOne({ id: h[i].userID })
            var tname;
...
async(Void, citel, text,{ isCreator }) => {
    ...
    const rudhra = "rudhra"
        ...
        const balance = await eco.balance(user1, rudhra);
        let a = (balance.wallet) < parseInt(word)
        //Returns wallet, bank, and bankCapacity. Also creates a USer if it doesn't exist.
        if(a == true) return citel.reply("you dont have sufficient money to transfer👎");

        const deduct = await eco.deduct(user1, rudhra, value[0]);
        const give = await eco.give(user2, rudhra, value[0]);
        let buttons = [{
            buttonId: `${prefix}wallet`,
            buttonText: {
                displayText: "Wallet👛",
...
Version: 0.0.6`, citel);
    ...
    async(Void, citel, text,{ isCreator }) => {
        ...
        if (mongoschemas == "false") return citel.reply("*🚦Economy* is not active in current group.");
         const rudhra = "rudhra"
         const balance = await eco.balance(citel.sender, rudhra); //Returns wallet, bank, and bankCapacity. Also creates a USer if it doesn't exist.
         let buttons = [{
...
Version: 0.0.6`, citel);
    ...
    async(Void, citel, text,{ isCreator }) => {
        if(!isCreator) return
         ...
         await eco.give(users, rudhra, parseInt(text.split(' ')[0]));
...
Version: 0.0.6`, citel);
	...
	const rudhra = "rudhra"
	    const k = 1000
        const balance1  = await eco.balance(user1, rudhra)
	const balance2  = await eco.balance(user2, rudhra)
    ...
    switch (random) {
	  ...
	  const deduff = Math.floor(Math.random() * 1000)	    
          await eco.deduct(user2, rudhra, deduff);
          await eco.give(citel.sender, rudhra, deduff);
        ...
        case 'caught':
           ...
           await eco.deduct(user1, rudhra, rmoney);
...
default:
 ...
 ///.mentionedJid[0] ? m.mentionedJid[0] : m.sender
     ...
     if(opp==='left')
     {
         hjkl = 'https://github.com/abhiru-dhachu/rudhra-md/blob/main/database/media/symbols/leftr.webp?raw=true'
     } 
    else if(opp==='right') 
    {
        hjkl = 'https://github.com/abhiru-dhachu/rudhra-md/blob/main/database/media/symbols/rightr.webp?raw=true'
    } else if(opp==='up') 
    {
        hjkl = 'https://github.com/abhiru-dhachu/rudhra-md/blob/main/database/media/symbols/upr.webp?raw=true'
    } else if (opp==='down'){
        hjkl = 'https://github.com/abhiru-dhachu/rudhra-md/blob/main/database/media/symbols/downr.webp?raw=true'
    } else{
        citel.reply(`Please provide direction(left,right,up,down).\nEg:- ${prefix}gamble 200 left`)
    }
   let media = await getBuffer(hjkl)
   citel.reply(media,{packname:'Rudhra',author:'Economy'},"sticker")
     ...
     if (!text) return citel.reply(
        ...
        if ( r == opp){
           let give = await eco.give(user , rudhra, twice);
           let buttons = [{
            buttonId: `${prefix}slot`,
            buttonText: {
                displayText: "Slot🎰",
...
Version: 0.0.6`, citel);
           ...
           let deduct = await eco.deduct(user, rudhra, texts[0]);
           let buttons = [{
            buttonId: `${prefix}slot`,
            buttonText: {
                displayText: "Slot🎰",
...
Version: 0.0.6`, citel);
    ...
    async(Void, citel, text,{ isCreator }) => {
        ...
        if (today.getDay() == 6 || today.getDay() == 5 || today.getDay() == 0){
            ...
            const rudhra = "rudhra"
            const k = 100
            const balance1  = await eco.balance(user, rudhra)
...
if(value<=balance.wallet){
    ...
    if ((f1 !== f2) && f2 !== f3){
        const deduct1 = await eco.deduct(user, rudhra, deduff);
        return citel.reply(`${mess1}\n\n*Big Lose -->* _🪙${deduff}_`)
     }
     else if ((f1 == f2) && f2 == f3){
        const give1 = await eco.give(user, rudhra, deduff/2);
        return citel.reply(`${mess2}\n*_Little Jackpot -->* _🪙${deduff/2}_`)
     }
     else if ((f1 == f2) && f2 !== f3){
        const give2 = await eco.give(user, rudhra, deduff);
        return citel.reply(`${mess3}\n*Small Win -->* _🪙${deduff}_`)
     }
     else if ((f1 !== f2) && f1 == f3){
        const deduct2 = await eco.deduct(user, rudhra, deduff);
        return citel.reply(`${mess5}\n\n*Small Lose -->* _🪙${deduff}_`)
     }
     else if ((f1 !== f2) && f2 == f3){
        const give4 = eco.give(user, rudhra, deduff);
        return citel.reply(`${mess3}\n\n*Small Win -->* _🪙${deduff}_`)
     }
     else if ((f1 == f2) && (f2 == f3) && (f3 == f4)){
        const give5 = eco.give(user, rudhra, deduff*20);
    ...
    return citel.reply('You don\'t have enough 💰amount in your👛 wallet.\n- Please don\'t provide 🤑amount.')
            ...
            if ((f1 !== f2) && f2 !== f3){
               const deduct1 = await eco.deduct(user, rudhra, 50);
                      citel.reply(`${mess1}\n\n*Big Lose -->* _🪙50_`)
            }
            else if ((f1 == f2) && f2 == f3){
               const give1 = await eco.give(user, rudhra, 100);
                     citel.reply(`${mess2}\n*_Little Jackpot -->* _🪙100_`)
            }
            else if ((f1 == f2) && f2 !== f3){
               const give2 = await eco.give(user, rudhra, 20);
                     citel.reply(`${mess3}\n*Small Win -->* _🪙20_`)
            }
            else if ((f1 !== f2) && f1 == f3){
               const deduct2 = await eco.deduct(user, rudhra, 20);
                     citel.reply(`${mess5}\n\n*Small Lose -->* _🪙20_`)
            }
            else if ((f1 !== f2) && f2 == f3){
               const give4 = eco.give(user, rudhra, 20);
                     citel.reply(`${mess3}\n\n*Small Win -->* _🪙20_`)
            }
            else if ((f1 == f2) && (f2 == f3) && (f3 == f4)){
               const give5 = eco.give(user, rudhra, 1000);
                    citel.reply(`${mess4}\n\n_🎊 JackPot --> _🪙1000_`)
            }
            else {
...
async(Void, citel, text,{ isCreator }) => {
    ...
    if (mongoschemas == "false") return citel.reply("*🚦Economy* is not active in current group.");
    const kg = 100
            const balance1  = await eco.balance(citel.sender, "rudhra")
    ...
    if(lol){
        ...
        const give2 = await eco.give(citel.sender, "rudhra", deduff*2);
...
Version: 0.0.6`, citel);
    ...
    const deduct1 = await eco.deduct(citel.sender, "rudhra", deduff);
    let st = `\n🎰 Slot Machine Result\n     ${i}\n\n      ${j}\n\n      ${k}\n\nNot Jacpot📉 but lost `
            let str = st.replace(/1/g, `🔴`).replace(/2/g, `🔵`).replace(/3/g, `🟣`).replace(/4/g, `🟢`).replace(/5/g, `🟡`).replace(/6/g, `⚪️`).replace(/7/g, `⚫️`).replace(/:/g, `    `)
    let buttons = [{
        buttonId: `${prefix}slot`,
