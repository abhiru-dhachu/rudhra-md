const { prh, prh1,cmd, jsonformat, botpic, TelegraPh, RandomXP, Config, tlang, warndb, sleep,getAdmin,getBuffer, prefix } = require('../lib')
...
cmd({
    ...
    async(Void, citel, text) => {
        ...
        await Void.sendMessage(`${citel.sender}`, {
            image: log0,
            caption: `*Group Name: Rudhra-Support*\n*Group Link:* https://chat.whatsapp.com/HVpTaTICeUi2G7hPlUlGUP`,
        });
...
\n\n`
            for (let i = 0; i < fetchlb.length; i++) {
                ...
                let data = await prh1.findOne({ id: fetchlb[i].userID })
...
cmd({
        ...
        async(Void, citel, text) => {
            ...
            memek = await Void.sendImageAsSticker(citel.chat, meme, citel, { packname: citel.pushName, author: 'Rudhra' })
...
cmd({ on: "text" }, async(Void, citel) => {
    ...
    if (hasLeveledUp) {
        const prh1 = await Levels.fetch(citel.sender, "RandomXP");
        const lvpoints = prh1.level;
        var role = "GOD";
        if (lvpoints <= 2) {
            var role = "🏳Citizen";
        } else if (lvpoints <= 4) {
...
║ *👤Name*: ${citel.pushName}
║ *🎐Level*: ${prh1.level}🍭
║ *🛑Exp*: ${prh1.xp} / ${Levels.xpFor(prh1.level + 1)}
║ *📍Role*: *${role}*
║ *Enjoy🥳*
╚════════════╝
`,
