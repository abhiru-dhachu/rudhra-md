const { addnote,cmd, prh1, delnote, allnotes, delallnote, tlang, botpic, runtime, prefix, Config } = require('../lib')
    //---------------------------------------------------------------------------
cmd({
            pattern: "addnote",
            category: "owner",
...
cmd({
            ...
            category: "owner",
            filename: __filename,
            desc: "Rudhra Qr code to scan and get your session id."
        },
...
cmd({
        ...
        async(Void, citel, text,{ isCreator }) => {
            ...
            try {
                ...
                prh1.findOne({ id: users }).then(async(usr) => {
                    if (!usr) {
                        console.log(usr.ban)
                        ...
                        await prh1.updateOne({ id: users }, { ban: "false" })
...
cmd({
        ...
        async(Void, citel, text,{ isCreator}) => {
            ...
            try {
                ...
                prh1.findOne({ id: users }).then(async(usr) => {
                    if (!usr) {
                        await new prh1({ id: users, ban: "true" }).save()
                        ...
                        await prh1.updateOne({ id: users }, { ban: "true" })
...
cmd({
        ...
        async(Void, citel, text, isAdmins) => {
            let alivemessage = Config.ALIVE_MESSAGE || `*A bot developed by princerudh.*`
            const alivtxt = `
